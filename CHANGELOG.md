...
#### Changed
- Configuring multiple `GLiNERRecognizer` instances via YAML with `class_name: GLiNERRecognizer` no longer requires an explicit unique `name` per entry — when omitted, a name is derived automatically from `class_name` and `model_name`. (#1760)
