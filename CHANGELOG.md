...
### Analyzer
#### Fixed
- Documented and added regression tests for configuring multiple `GLiNERRecognizer` instances via the recognizer registry YAML (e.g. different models/thresholds/entity mappings side by side). This already worked by giving each entry a distinct `name` while pointing `class_name` at `GLiNERRecognizer`, but it wasn't documented, so users hit the (undocumented) `class_name`/`name` mechanism used elsewhere for `HuggingFaceNerRecognizer`. Added a YAML example to the GLiNER sample doc and to `recognizer_registry_provider.md`. (#1760)
