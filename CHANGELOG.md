...
### Analyzer
#### Fixed
- Prevented `SlimSpacyNlpEngine` from registering an inert spaCy recognizer and advertising unsupported spaCy NER entities. The new `NlpEngine.has_ner` property defaults to `False`; custom engines that produce native NER output must ensure it returns `True` so Presidio registers an NLP recognizer.
